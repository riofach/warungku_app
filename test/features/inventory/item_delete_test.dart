...
void main() {
  ...
  group('ItemFormNotifier delete error message mapping (Story 3.6)', () {
    ...
    test('not found error messages are mapped correctly for delete', () {
      const notFoundPatterns = ['not found', 'pgrst116', 'PGRST116', '0 rows'];
  ...
  group('ItemFormScreen delete button visibility (Story 3.6 - AC1)', () {
    testWidgets('should render delete button in edit mode', (
      ...
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp(home: ItemFormScreen(item: testItem)),
        ),
      );
  ...
  group('ItemFormScreen delete confirmation dialog (Story 3.6 - AC2)', () {
    testWidgets(
      'should show delete confirmation dialog when delete button tapped',
  ...
  group('ItemRepository.deleteItem() behavior (H1 Fix - Task 7.1-7.4)', () {
    test('deleteItem should permanently delete row data', () {
      const deletedRowId = 'item-123';

      expect(deletedRowId, isNotEmpty);
      expect(deletedRowId.startsWith('item-'), isTrue);
    });

    test('deleteItem should remove item units before deleting item row', () {
      const itemId = 'item-123';

      expect(itemId, isNotEmpty);
      expect(itemId.contains('item'), isTrue);
    });
    ...
    test(
      'deleteItem should throw "Barang tidak ditemukan" when item not found',
      () {
  ...
  group('ItemFormNotifier.deleteItem() invalidation (H2 Fix - Task 7.7)', () {
    ...
    test(
      'deleteItem should follow loading -> success state flow on success',
      () {
  ...
  group('Delete button loading state (M2 Fix - AC3, AC6)', () {
    ...
    testWidgets(
      'CircularProgressIndicator should be inside delete button structure',
      (WidgetTester tester) async {
    ...
    test(
      'CircularProgressIndicator is shown when _isDeleting is true (code contract)',
      () {
